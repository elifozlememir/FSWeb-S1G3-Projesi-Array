...
if(newarr.length ==25) return true;
else return false;
...
function cesitEkle(arr,ad){
  let newarr = arr;
   newarr.splice(0,0,ad);
  return newarr;
  }
...
console.log(indekstekiCesitiGetir(orijinalTatlar,30))
...
function ismeGoreCesitCikar(arr,text){
  let newarr =arr;
...
function ismeGoreFiltrele(arr,text) {

  let newarr = [];
  for (let i = 0; i < arr.length; i++) {
    
    if (arr[i].includes(text) == true) {
     newarr.push(arr[i]);
    }
  }
  return newarr;
...
function ortalamaKelimeSayisi(arr){
  ...
  toplam = toplam + arr[i].split(" ").length ;
    }
...
function rastgeleTatlar(dizi1,dizi2,dizi3,dizi4,item){
let total=dizi1.concat(dizi2,dizi3,dizi4);
let newMenu=[];

for (let i=0;i<item;i++){
let rannum = Math.round(Math.random() * (total.length-1));

  newMenu.push(total[rannum]);
  total.splice(rannum,1);
}
